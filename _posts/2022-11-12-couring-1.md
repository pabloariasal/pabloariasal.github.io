...
The source code for the entire series can be found [here]().
...
The big advantage of the io_uring implementation is the substantial reductin of syscalls performed. In the synchronous implementations we made a `read` syscall for every file, now we make a single syscall for all the requests in the queue. In fact, using `strace` we can see that the io_uring implementation has 512 less system calls than the synchronous ones, this is due to the `read` syscalls:

```
> strace -c -e read -- ./build_release/own --trivial
Running trivial
Processed 512 files.
...
------ ----------- ----------- --------- --------- ----------------
100.00    0.000591           1       517           total

> strace -c -e read -- ./build_release/own --iouring
Running iouring
Processed 512 files.
% time     seconds  usecs/call     calls    errors syscall
------ ----------- ----------- --------- --------- ----------------
...
In the next parts of the series we will try to solve these problems. [Continue with part 2]().
