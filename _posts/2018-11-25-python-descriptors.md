...
Are called *descriptors*. If an objects implements all three, like `property`, it's called a *data descriptor*. If only `__get__` is present, the object is called a *non-data descriptor*.
